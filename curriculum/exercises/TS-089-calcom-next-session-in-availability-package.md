...
- `apps/api/v2/src/modules/slots/slots-2024-09-04/services/slots.service.ts` is a Nest service that calls `availableSlotsService.getAvailableSlots({ input: queryTransformed, ctx: {} })`, showing the availability engine is reused outside tRPC/Next sessions for API tokens, OAuth clients, and jobs.
