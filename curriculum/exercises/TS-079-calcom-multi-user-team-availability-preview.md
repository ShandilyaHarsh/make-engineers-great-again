...
A strong reviewer traces the request as a sequence of gates and side effects: router entry, membership check, member query, visibility boundary, credential hydration, provider calls, aggregation, and response. The reason for that order is practical: the first irreversible or expensive side effect should come after the smallest authorization boundary that can safely narrow the work. When calendar credentials are hydrated or provider calls are made, the reviewer should be able to point to the exact predicate that already limited the member set.
